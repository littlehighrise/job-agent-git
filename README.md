...
The workflow discovers and scores jobs only. It does **not** submit applications, drive a browser, call an LLM API, or require an OpenAI API key. Public Greenhouse board validation uses temporary workflow data: a generated search-preferences file, temporary SQLite database, temporary application/review artifacts, discovery logs, source-result metadata, and a machine-readable `live-validation-summary.json`. The live summary includes parsing-quality counts, explicit-requirement/responsibility counts, requirement-evaluation counts, non-zero evidence-confidence counts, hard-requirement presence counts, absent scoring-dimension counts, below-threshold-only rejection counts, near-review-threshold counts, and prominent non-fatal warnings when every fetched job parses as insufficient or no jobs produce requirements/evaluations. User-facing Top Matching Jobs prioritize reviewable or auto-eligible jobs and then rejected jobs in the target role family; unrelated rejected roles are kept in a separate highest-scoring rejected diagnostic section.
